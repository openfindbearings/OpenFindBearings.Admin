# Admin 统一管理平台总体设计 v1.3.0

## 概述

OpenFindBearings.Admin 是一个大一统的超级管理后台，将 Crawler、Sync、API 数据、Identity 用户权限的运维管理整合到单一 ASP.NET Core MVC 项目中。Phase 1 阶段不启用认证，所有业务数据通过调用各服务的 HTTP API 操作。Admin 拥有独立的 PostgreSQL 数据库（db_admin），用于存储 RBAC 角色权限映射和审计日志。
