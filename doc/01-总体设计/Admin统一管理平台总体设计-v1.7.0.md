# Admin 统一管理平台总体设计 v1.7.0

## 概述

OpenFindBearings.Admin 是一个大一统的超级管理后台，将 Sync、API 数据、Identity 用户权限的运维管理整合到单一 ASP.NET Core MVC 项目中。认证使用 OAuth 授权码流程 + BearerTokenHandler JWT 注入。Admin 拥有独立的 PostgreSQL 数据库（db_admin），用于存储 RBAC 角色权限映射和审计日志。

Crawler 项目以纯 CronJob 模式运行（不常驻 Web 实例），Admin 不提供远程控制能力，仅通过 Sync 的数据源端点只读展示已接入的爬虫数据源。
...
| 爬虫管理 | 只读展示（数据源清单 + 静态调度策略） | Crawler 纯 CronJob 零常驻，不支持手动触发/状态/运行历史/调度修改 |
...
│   │
│   ├── Controllers/
│   │   ├── AccountController.cs       — 登录/回调/登出/修改密码/Profile/SignoutCallbackOidc
│   │   ├── HomeController.cs          — 仪表盘 + /Home/Status + /Home/DataSources 端点 + 爬虫只读入口
...
│   │   ├── MerchantVerifyController.cs — 商家认证审核（含 reject）
│   │   ├── UsersController.cs         — 用户管理（调 Identity，含 unlock/hard-delete）
│   │   ├── RoleController.cs          — 角色管理（db_admin RBAC）
│   │   ├── PermissionController.cs    — 权限管理（db_admin RBAC）
│   │   ├── AuditLogController.cs      — 审计日志（三服务源聚合）
...
│   │   ├── Entities/                  — 4 个实体类
│   │   ├── Enums/
│   │   │   ├── PermissionKey.cs       — 17 个权限键
│   │   │   └── AdminAction.cs         — 审计动作枚举
│   │   └── ViewModels/                — 10 个 ViewModel 类
│   │
│   ├── Services/
│   │   ├── BearerTokenHandler.cs      — JWT 自动注入（cookie → Authorization header）
│   │   ├── AdminTokenService.cs       — Token 刷新 + 按 device_id 缓存 + Cookie 回写
│   │   ├── PriceConfigService.cs      — 价格配置缓存（Singleton，5 分钟过期）
│   │   └── ServiceHealthService.cs    — 3 服务健康检查（/health/live 端点）
│   │
│   ├── Views/
│   │   ├── Shared/_Layout.cshtml      — 固定侧边栏 + 顶栏 + 主题切换
│   │   ├── Home/Index.cshtml          — 仪表盘（4 统计卡片 + 待审核数 + 服务状态）
│   │   ├── Data/                      — 轴承/品牌/类型/商家列表（CRUD 弹窗）+ ImportInventory
│   │   ├── Crawler/Index.cshtml       — 数据爬虫（只读展示）
...
│   │   ├── Role/Index.cshtml          — 角色管理
│   │   ├── Role/Permissions.cshtml    — 角色权限分配
│   │   ├── Permission/Index.cshtml    — 权限管理
│   │   ├── AuditLog/Index.cshtml      — 审计日志（三服务 tab 切换）
...
│   │
│   └── deploy/k3s/                    — deployment + service + ingress + secret + kustomization
│
└── doc/                               — 设计文档
```
...
- **editor（业务员）**：轴承/商家 CRUD + 纠错审核 + 系统配置查看
- **viewer（审计员）**：仪表盘/审计日志/系统配置只读

### 权限键（PermissionKey）

共 17 个权限键，使用 `bearing.*` 小写点命名：

```
dashboard.view, bearing.view, bearing.create, bearing.edit, bearing.delete,
merchant.view, merchant.manage, merchant.verify, correction.review,
etl.manage,
role.manage, user.manage,
system.view, system.manage, audit.view,
data.restore, data.harddelete
```

## 健康检查

ServiceHealthService 并行检查 3 个服务（API、Sync、Identity）的 `/health/live` 端点（统一路径），非阻塞初始化。Crawler 为纯 CronJob 无常驻实例，不在健康检查范围。
...
| v1.6.0 | 2026-08-03 | 爬虫管理降级为只读展示：移除 CrawlerController、Crawler 健康检测、Crawler 审计日志源、crawler.manage 权限；健康检查收敛为 3 服务；数据访问原则改为经 Sync 读取数据源清单；项目结构/导航/权限键同步 |
| v1.7.0 | 2026-09-11 | 代码审查对齐：① DTO 数量 13+→16；② ViewModel 数量 5→10；③ Services 补充 AdminTokenService/PriceConfigService；④ Views 补充 Role/Index+Permissions、Account/Profile、Data/ImportInventory；⑤ Controllers 补充 RoleController（非 RolesController）、AccountController.Profile+SignoutCallbackOidc；⑥ deploy 路径修正为项目根目录；⑦ 补充 ingress.yaml/secret.yml 等部署文件；⑧ 数据访问原则中纠错/营业执照/商家认证端点路径修正为 /api/admin/*；⑨ 权限键 v1.5.0 changelog 中 21 个修正为 17 个 |
