...
Admin 通过 HTTP API 调用 4 个后端服务。Phase 1 已通过 BearerTokenHandler 自动从登录 cookie 中提取 JWT 注入到 API 请求。本文档记录每个服务的端点清单、调用方式及 Admin 侧的 HttpClient 配置。

## 通用约定

- 认证：通过 BearerTokenHandler 自动从 cookie `access_token` claim 注入 `Authorization: Bearer {token}`（仅 ApiClient）
- 内容类型：`application/json`（文件上传使用 `multipart/form-data`）
- 超时：API/Crawler/Sync 30 秒，Identity 10 秒
- 错误处理：非 2xx 响应 → 返回空数据 + 页面显示降级提示
- 命名 HttpClient：ApiClient（带 BearerTokenHandler）、SyncClient、IdentityClient

## 1. Crawler API（:5078，仅 web 模式）

### 现有端点

Crawler 项目内置 3 个 Minimal API 管理端点（内存 CrawlerRunTracker 驱动），在 web 模式下可用；K8s 生产环境 Crawler 为纯 CronJob（`--task=crawl`），不暴露 web 实例，这些端点不被外部访问。
...
Admin 早期通过 `CrawlerController` 调用上述端点实现爬虫管理；现爬虫管理已降级为**只读展示**：Admin「数据爬虫」页不再调用 Crawler，而是经 Sync `GET /api/datasources`（按 Type==Database 过滤）动态渲染已接入的爬虫数据源清单。因此 Admin 已移除 `CrawlerClient` 注册与 `ApiUrls:FindBearingsCrawler` 配置项，Crawler 本地 web 默认端口为 `https://localhost:5078`（http 为 7138）。
...
| POST | `/api/sync/excel/bearing` | Excel 批量导入在售轴承 |
| GET | `/api/sync/excel/template` | 下载 Excel 导入模板 |
...
Admin 在 `Program.cs` 中注册了 4 个代理端点，供前端 AJAX 直接调用：
...
| v1.4.1 | 2026-08-27 | 修正第 1 节 Crawler API：明确端点仅 web 模式可用、K8s 纯 CronJob 不暴露；Admin 已删除 CrawlerController 与 CrawlerClient，爬虫页改为只读（经 Sync /api/datasources 拉取）；同步移除通用约定中失效的 CrawlerClient 命名与 7207 错误端口 |
