...
| DELETE | `/api/admin/bearings/{id}/hard` | 彻底删除轴承（需 DataHardDelete 权限） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/admin/corrections` | 纠错列表（分页+状态筛选） |
| POST | `/api/admin/corrections/{id}/approve` | 审批通过 |
| POST | `/api/admin/corrections/{id}/reject` | 审批拒绝 |

#### 营业执照

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/admin/licenses/pending` | 营业执照待审列表 |
| POST | `/api/admin/licenses/{id}/approve` | 通过 |
| POST | `/api/admin/licenses/{id}/reject` | 拒绝 |

#### 商家认证

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/admin/merchants/{id}/verify` | 认证商家 |
| POST | `/api/admin/merchants/{id}/reject` | 拒绝认证 |
...
Admin 在 `Program.cs` 中注册了 7 个代理端点，供前端 AJAX 直接调用：
...
// 轴承在售商家反向查询代理
app.MapGet("/api/proxy/bearing-merchants/{bearingId:guid}", ...);
...
// ETL 摘要查询代理（转发到 Sync API）
app.MapGet("/api/proxy/etl/summary", ...);

// ETL 任务列表查询代理（转发到 Sync API）
app.MapGet("/api/proxy/etl/tasks", ...);
```
...
| POST | `/api/account/admin/users/{id}/unlock` | 解锁账户 |
| POST | `/api/account/admin/users/{id}/reset-password` | 重置密码 |
| DELETE | `/api/account/admin/users/{id}/permanent` | 物理删除（仅已软删） |
...
| v1.6.0 | 2026-09-11 | 代码审查对齐：① 纠错端点路径修正为 /api/admin/corrections；② 营业执照端点路径修正为 /api/admin/licenses/pending；③ 商家认证补充 reject 端点；④ 代理端点补充 bearing-merchants/etl/summary/etl/tasks（4→7）；⑤ Identity 用户管理补充 unlock/permanent/restore 端点；⑥ 轴承管理补充 hard delete 端点 |
