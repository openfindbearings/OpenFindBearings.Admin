...
> 系统配置现为后台可调的权威来源，除原有基础/价格/缓存/限流等键外，本次新增：`Reliability.AutoSyncThreshold`（自动同步阈值）、`Reliability.ReviewThreshold`（人工审核阈值）、`Reliability.DefaultSourceScore`（来源默认基础分）、`Site.BeiAn`（备案号）、`Site.CustomerService`（客服联系方式）。其中可信度三阈值由 Sync 运行时经 `GET /api/config/reliability` 拉取，Admin 编辑后约 5 分钟内 Sync 自动生效，无需重启。
