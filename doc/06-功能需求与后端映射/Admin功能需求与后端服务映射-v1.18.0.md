...
| 价格配置（Admin 缓存读取） | API | `GET /api/admin/config/price` | ✅ |

**配置分组（3 组，13 项，均有真实消费方）**

| 分组 | 配置键 | 消费方 |
|------|--------|--------|
| 站点设置 | `SiteName` / `SiteDescription` / `Site.BeiAn` / `Site.CustomerService` | `GetMobileConfigQueryHandler`，供 MAUI 端展示 |
| 价格显示 | `Price.DefaultVisibility` / `Price.ShowNegotiableLabel` / `Price.NumericForSorting` / `Price.ExtractPattern` | Admin 在售商品弹窗（前两项）+ API 商品创建/导入与排序（后两项） |
| 数据同步 | `RateLimit.Guest/User/Premium.RequestsPerMinute` / `Reliability.AutoSyncThreshold` / `Reliability.ReviewThreshold` / `Reliability.DefaultSourceScore` | `RateLimitingMiddleware` + Sync 运行时经 `GET /api/config/reliability` 拉取 |

> 系统配置现为后台可调的权威来源。可信度三阈值由 Sync 运行时经 `GET /api/config/reliability` 拉取，Admin 编辑后约 5 分钟内 Sync 自动生效，无需重启。价格配置中 `ShowNegotiableLabel` 与 `NumericForSorting` 由 Admin 的 `PriceConfigService` 缓存读取（5 分钟 TTL，保存配置时主动失效）；`ExtractPattern` 与 `DefaultVisibility` 由 API 的 `IPriceConfigProvider` 缓存读取（5 分钟 TTL）。
>
> 已删除的无消费方配置项：`EnableRegistration` / `RequireEmailVerification` / `DefaultUserRole` / `ItemsPerPage` / `Cache.*`——这些键自创建起无任何代码读取，保留会让管理员误以为调整后有效果。
...
| v1.17.0 | 2026-08-29 | 系统配置新增可信度三阈值（Reliability.AutoSyncThreshold / ReviewThreshold / DefaultSourceScore，供 Sync 运行时经 GET /api/config/reliability 拉取）与站点展示项（Site.BeiAn / Site.CustomerService）；相应精简 Sync appsettings 与 K8s ConfigMap 中的阈值键，单一权威来源收敛到 API SystemConfigs |
