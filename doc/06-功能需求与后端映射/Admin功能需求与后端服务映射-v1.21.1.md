...
2. **抽屉材料卡重构**：缩略图主导（110px cover 裁切）、PDF 显文件图标、点缩略图弹 **lightbox** modal 大图；加载失败显示占位文案不再隐身。
3. **抽屉状态/渠道映射大小写归一**：API 输出 status 为 PascalCase、applicationMode 为小写，原查表只匹配一种形态导致"状态 -"；归一后查表修复。
