...
- v1.24.0 商户管理页新增"解除归属"操作（POST /api/admin/merchants/{id}/detach，权限键 merchant.detach 迁移补插存量库）：认领商户清场回公海（商品/证照/成员/邀请/纠错全清，随爬取自然更新、可再认领），自建商户被 API 拒绝并透传原因；data-confirm-danger 二次确认；权限分组与中文名两处映射同步。
