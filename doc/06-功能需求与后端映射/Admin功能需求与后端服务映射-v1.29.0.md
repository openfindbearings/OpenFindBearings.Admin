...
- v1.29.0 用户/角色页改版：用户列表角色列接通 API 平台角色（批量 platform-roles 端点合并，徽章显中文显示名）+ 全部/后台用户/App 用户三页签（当前页过滤，.ofb-tabs 公共皮肤移动端横滑）；角色管理显示 DisplayName 主显+英文标识副标、新建弹窗标识/显示名分离（标识前端预检英文正则）；平台角色弹窗目录带中文名；系统配置页签套用 .ofb-tabs；修复角色删除守卫小写 admin 恒不命中与 GetRoleDetail 基类强转派生类 500 两处存量缺陷
