...
| 功能点 | 后端 | 端点 | 实现状态 |
|--------|------|------|----------|
| 品牌列表（分页+搜索） | API | `GET /api/brands?page=&search=&includeDeleted=` | ✅ |
...
| 类型列表（分页+搜索） | API | `GET /api/bearing-types?page=&search=&includeDeleted=` | ✅ |
...
| 信息纠错列表 | API | `GET /api/corrections` | ✅ |
| 信息纠错通过 | API | `POST /api/corrections/{id}/approve` | ✅ |
| 信息纠错拒绝 | API | `POST /api/corrections/{id}/reject` | ✅ |
| 营业执照列表 | API | `GET /api/merchant-licenses` | ✅ |
| 营业执照审核通过 | API | `POST /api/merchant-licenses/{id}/approve` | ✅ |
| 营业执照审核拒绝 | API | `POST /api/merchant-licenses/{id}/reject` | ✅ |
| 商家认证搜索 | API | `GET /api/merchants/search?unverified=true` | ✅ |
| 商家认证审核 | API | `POST /api/admin/merchants/{id}/verify` | ✅ |
...
| 功能点 | 后端 | 端点 | 实现状态 |
|--------|------|------|----------|
| 爬虫列表 | Crawler API | `GET /api/crawlers` | ✅ |
| 爬虫状态 | Crawler API | `GET /api/crawlers/{name}/status` | ✅ |
| 触发运行 | Crawler API | `POST /api/crawlers/{name}/run` | ✅ |
| 爬虫运行历史 | — | — | ❌ |
...
| 审计日志（Crawler） | Crawler API | `GET /api/audit-log?source=crawler` | ✅ |

### 第五层：系统配置

| 功能点 | 后端 | 端点 | 实现状态 |
|--------|------|------|----------|
| 配置列表 | API | `GET /api/config` | ✅ |
| 编辑配置 | API | `PUT /api/config/{key}` | ✅ |
...
| MerchantVerifyController | 商家认证审核 | API |
| UsersController | 用户管理 | Identity |
| PermissionController | 权限管理（db_admin RBAC） | db_admin |
| AuditLogController | 审计日志（四服务 tab 切换） | API/Sync/Crawler/Admin |
...
| ServiceHealthService | 并行检查 4 服务 /health/live 端点 |
