...
| 组件 | 说明 |
|------|------|
| Program.cs | 4 命名 HttpClient（ApiClient 带 BearerTokenHandler）+ ServiceHealthService + 4 代理端点（支持 dataSource 和 onlyOnSale 参数） |
| ApplicationDbContext | EF Core DbContext（4 个 DbSet）+ PostgreSQL |
| SeedData | 3 角色（admin/editor/viewer）+ 18 权限键种子数据 |
